...
contract TestDestinationMinter is Test {
    ...
    function test__DestinationContractInitialization() public view {
        RandomizedNFT nftContract = RandomizedNFT(destinationMinter.getNftContractAddress());
        assertEq(nftContract.supportsInterface(0x80ac58cd), true);
        assertEq(destinationMinter.getRouterAddress(), networkConfig.destinationRouter);
    ...
    function test__SetRoyalty() public {
        ...
        uint256 salePrice = 100;
        (address feeAddress, uint256 royaltyAmount) = nfts.royaltyInfo(0, salePrice);
